...
# Session Dataframe
# ----------------------------------------------------------------------------------
def rename_compat(df:pd.DataFrame):
    # rename the column to be compatiable with v1.8.1
    return df.rename(columns={'content':'value', 'subtype':'name'})


def session_dataframe(file_path, paired_events={}, pair_end_suffix=None, time_unit="ms"):

...
def get_task_specs(tasks_trig_and_events, task_name):
    ...
    triggers = (
        np.array2string(tasks_trig_and_events["triggers"][task_idx].values)
        .strip("'[]")
        .split("; ")
    )
    if triggers[0] == "":
    ...
    # events to extract
    events_to_process = (
        np.array2string(tasks_trig_and_events["events"][task_idx].values)
        .strip("'[]")
        .split(";")
    )
    ...
    conditions = (
        np.array2string(tasks_trig_and_events["conditions"][task_idx].values)
        .strip("'[]")
        .split(";")
    )
...
def extract_trial_by_trigger(
    df_pycontrol, trigger, event2analysis, trial_window, subject_ID, datetime_obj
):
    ...
    df_events, trigger_time = add_trial_nb(
        df_events, trigger_time, trial_window
    )  # add trial number according to the trigger

    # if len(trigger_time) == 0:
    #     logging.warning('No trial can be found. I am going to treat the whole session as one single trial')
    #     trigger_time = df_events.time[0]-trial_window*2 # make sure we have enough data

    df_events = df_events.groupby("trial_nb", group_keys=False).apply(
        get_rel_time, trigger_name=trigger
    )
    ...
    df_events_trials["uid"] = df_events_trials["trial_nb"].apply(
        lambda x: f"{subject_ID}_{datetime_obj.date()}_{datetime_obj.time()}_{x}"
    )
    ...
    if len(df_events_trials) > 0 and (
        df_events_trials["timestamp"].iloc[0] > abs(trial_window[0])
    ):
...
def compute_success(df_events_trials, df_cond, task_name, triggers=None, timelim=None):
    ...
    # To perform for cued-uncued version of the go task
        ...
        if (
            "spout_trial_time" in df_events.columns
            and "US_end_timer_trial_time" in df_events.columns
        ):
...
class Session:
    ...
    def get_photometry_trials(
        ...
        if not hasattr(self, "photometry_rsync"):
            raise Exception(
                "The session has not been matched with a .ppd file, \
                please run experiment.match_to_photometry_files(kvargs)"
            )
        elif self.photometry_rsync == None:
            raise Exception(
                "The session has no matching .ppd file, or no alignment \
        ...
        if (
            ...
            and high_pass == None
            and low_pass == None
            and median_filt == None
        ):
            raise Exception(
                "You need to high_pass and/or low_pass and/or median_filt the signal for motion correction"
            )
        ...
        photometry_dict = import_ppd(
            self.files["ppd"][0],
            high_pass=high_pass,
            low_pass=low_pass,
            medfilt_size=median_filt,
        )
        ...
        if motion_corr == True:
            ...
            if df_over_f == False:
                ...
                b, a = butter(
                    2, 0.001, btype="low", fs=photometry_dict["sampling_rate"]
                )
                photometry_dict["analog_1_baseline_fluo"] = filtfilt(
                    b, a, photometry_dict["analog_1_filt"], padtype="even"
                )

                # Now calculate the dF/F by dividing the motion corrected signal by the time varying baseline fluorescence.
                photometry_dict["analog_1_df_over_f"] = (
                    photometry_dict["analog_1_corrected"]
                    / photometry_dict["analog_1_baseline_fluo"]
                )
        ...
        if downsampling_factor:
            ...
            photometry_dict["sampling_rate"] = (
                photometry_dict["sampling_rate"] / downsampling_factor
            )

        fs = photometry_dict["sampling_rate"]

        df_meta_photo = pd.DataFrame(
            columns=["subject_ID", "datetime", "task_name", "condition_ID", "trial_nb"]
        )
        ...
        for condition_ID, conditions_dict in enumerate(conditions_list):
            ...
            trials_idx, timestamps_pycontrol = self.get_trials_times_from_conditions(
                conditions_dict, trig_on_ev=trig_on_ev, last_before=last_before
            )
            ...
            photometry_idx = (
                (timestamps_photometry / (1000 / photometry_dict["sampling_rate"]))
                .round()
                .astype(int)
            )
            ...
            complete_mask = (
                photometry_idx
                + trial_window[0] / (1000 / photometry_dict["sampling_rate"])
                >= 0
            ) & (
            ...
            photometry_idx = [
                range(
                    idx
                    + int(trial_window[0] / (1000 / photometry_dict["sampling_rate"])),
                    idx
                    + int(trial_window[1] / (1000 / photometry_dict["sampling_rate"])),
                )
                for idx in photometry_idx
            ]
            ...
            if condition_ID == 0:
                ...
                photo_array = np.ndarray(
                    (len(trials_idx), len(photometry_idx[0]), len(export_vars))
                )
                ...
                for var_idx, photo_var in enumerate(export_vars):
                    ...
                    photo_array[:, :, var_idx] = np.take(
                        photometry_dict[photo_var], photometry_idx
                    )
                ...
                photo_array_temp = np.ndarray(
                    (len(trials_idx), len(photometry_idx[0]), len(export_vars))
                )

                for var_idx, photo_var in enumerate(export_vars):
                    # print(f'condition {condition_ID} var: {var_idx} shape {np.take(photometry_dict[photo_var], photometry_idx).shape}')
                    photo_array_temp[:, :, var_idx] = np.take(
                        photometry_dict[photo_var], photometry_idx
                    )

                df_meta_photo_temp = pd.DataFrame(
                    columns=[
                        "subject_ID",
                        "datetime",
                        "task_name",
                ...
                df_meta_photo = pd.concat(
                    [df_meta_photo, df_meta_photo_temp], axis=0, ignore_index=True
                )
                # concatenate with previous numpy array

                # Take into account when there is no trial in first condition
                if "photo_array" in locals():
                    photo_array = np.concatenate(
                        (photo_array, photo_array_temp), axis=0
                    )
                else:
        ...
        if "photo_array" in locals():
            ...
            if verbose:
                print(
                    f"No photometry data to collect for subject ID:{self.subject_ID}\
    ...
    def get_trials_times_from_conditions(
        ...
        if conditions_dict is not None:
            ...
            for idx_cond, cond in enumerate(cond_pairs):
                ...
                idx_rows.append(
                    set(
                        self.df_conditions.index[
                            self.df_conditions[cond_pairs[idx_cond][0]]
                            == cond_pairs[idx_cond][1]
                        ].values
                    )
                )
        ...
        # Otherwise offset trigger timestamp by occurence of first event (TODO: or last before)
            ...
            if last_before is not None and last_before in set(self.events_to_process):
                ...
                if (
                    len(idx_joint) == 0
                ):  # Added because find_last_time_before_list did not do well with empty Df
                    ...
                    ev_times = df_ev_copy.loc[(idx_joint), [ev_col, before_col]].apply(
                        lambda x: find_last_time_before_list(x[ev_col], x[before_col]),
                        axis=1,
                    )
            ...
            # If last_before is not requested
                ...
                ev_times = df_ev_copy.loc[
                    (idx_joint), trig_on_ev + "_trial_time"
                ].apply(lambda x: find_min_time_list(x))
    ...
    def compute_behav_metrics(self, conditions_dict, events=None):
        ...
        if events:
            ...
            if any([ev not in self.events_to_process for ev in events]):
                raise Exception(
                    'One or more requested events are not belonging to the "events_to_process"'
                )

        idx_cond, trial_times = self.get_trials_times_from_conditions(
        ...
        if self.analyzed == False:
            print(
                "behavioural metrics impossible to calculate, file is too short or not extracted by trials:",
                self.subject_ID,
            ...
            try:
                if "trigger" in list(conditions_dict.keys()):
                    success_rate = len(
                        self.df_conditions[
                            (self.df_conditions["valid"] == True)
                            & (self.df_conditions["success"] == True)
                            & (
                                self.df_conditions["trigger"]
                                == conditions_dict["trigger"]
                            )
                        ].index
                    ) / len(
                        self.df_conditions[
                    ...
                    success_rate = len(
                        ...
                        self.df_conditions[(self.df_conditions["valid"] == True)].index
                    )
            ...
            if events is not None:
                ...
                for ev in events:
                    ...
                    mean_min_time = (
                        self.df_events[ev + "_trial_time"]
                        .loc[idx_cond]
                        .apply(lambda x: min([i for i in x if i > 0], default=np.nan))
                        .mean()
                    )
                    std_min_time = (
                        self.df_events[ev + "_trial_time"]
                ...
                if len(events) == 2:
                    ...
                    col_idx_start = (
                        list(self.df_events.columns.values).index(
                            events[0] + "_trial_time"
                        )
                        + 1
                    )  # +1 since index is the first value of the row from itertuples()
                    col_idx_end = (
                        list(self.df_events.columns.values).index(
                            events[1] + "_trial_time"
                        )
                        + 1
                    )  # +1 since index is the first value of the row from itertuples()
                    time_intervals = []
                    for row in self.df_events.loc[idx_cond].itertuples():
                        time_intervals.append(
                            time_delta_by_row(row, col_idx_start, col_idx_end)
                        )
